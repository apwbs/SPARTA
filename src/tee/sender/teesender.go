...
func SendSeed(expectedMeasurement string, SKIPATTESTATION bool) {
	...
	if !SKIPATTESTATION {
		...
		certBytes, _ = attestation.RemoteAttestation("https://localhost:8075", []byte(expectedMeasurement), "../teeserver/pubkey/publicKey.pem")
	} else {
		fmt.Println("Skipping remote attestation")
		tlsConfig := &tls.Config{InsecureSkipVerify: true}
		certBytes = https.HttpGet(tlsConfig, "https://localhost:8075/cert")
	}
	...
	req, err := http.NewRequest("POST", "https://localhost:8075/secret", &requestBody)
	if err != nil {
